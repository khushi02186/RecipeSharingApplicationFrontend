...
const CreateRecipe = () => {
    ...
    const onSubmit = async (data) => {
        try {
            ...
            const formattedData = {
                ...data,
                steps: data.steps.map(step => step.value || step), // Handle both object and string just in case
                userId: user.id,
            ...
            if (isEditMode) {
                ...
                await api.post('/recipe/add', formattedData);
            }
            navigate('/');
        } catch (error) {
    ...
    return (
        <div className="min-h-screen bg-white dark:bg-dark text-gray-900 dark:text-light py-12 px-4 sm:px-6 lg:px-8 transition-colors duration-200">
            <div className="max-w-4xl mx-auto">
                <div className="flex items-center mb-8">
                    ...
                    <h1 className="text-3xl font-bold text-gray-900 dark:text-white">{isEditMode ? 'Edit Recipe' : 'Create New Recipe'}</h1>
                </div>
